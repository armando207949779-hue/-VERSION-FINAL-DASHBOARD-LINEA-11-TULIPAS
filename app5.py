"""
Dashboard de Mantenimiento de Tulipas - Embotelladora CCU Línea 11
Streamlit App para GitHub + Streamlit Cloud
Conectado a Google Sheets
VERSIÓN 5.0 LÍNEA 11: ajustada a nueva planilla Google Sheets y carga robusta
"""
...
def normalizar_formato(val):
    """Normaliza variantes de formato al valor canónico"""
    if pd.isna(val):
        return val
...
def load_data_from_sheets():
    ...
    try:
        ...
        if 'Fecha registro' not in df.columns:
            df['Fecha registro'] = pd.NaT

        df['Fecha'] = pd.to_datetime(df['Fecha'], dayfirst=True, errors='coerce')
        df['Fecha registro'] = pd.to_datetime(df['Fecha registro'], errors='coerce')

        # Si Fecha viene vacía, intenta completar con la fecha de Fecha registro
        mask_fecha_vacia = df['Fecha'].isna() & df['Fecha registro'].notna()
        df.loc[mask_fecha_vacia, 'Fecha'] = df.loc[mask_fecha_vacia, 'Fecha registro'].dt.normalize()

        # Limpieza básica de texto
        for col in ['Turno', 'Operador', 'Equipo', 'Formato', 'Mantención', 'Comentarios']:
            df[col] = df[col].astype(str).str.strip()
            df[col] = df[col].replace({'nan': '', 'None': ''})

        df['Operador'] = df['Operador'].replace('', 'Sin operador')
        df['Turno'] = df['Turno'].replace('', 'Sin turno')
        ...
        df['Equipo'] = df['Equipo'].replace({
            'encajonadora': 'Encajonadora',
            'desencajonadora': 'Desencajonadora'
        })

        df = df.dropna(subset=['Fecha', 'Cabezal', 'Tulipa'])
        df = df.sort_values(['Fecha', 'Cabezal', 'Tulipa'])
...
def load_data_example():
    np.random.seed(42)
    fechas = pd.date_range('2025-01-01', '2026-04-10', freq='D')
    n = 2500
    data = {
        ...
        'Mantención': np.random.choice([
            'CAMBIO DE VÁSTAGO', 'CAMBIO CUERPO TULIPA PLÁSTICA', 'CAMBIO DE RESORTE',
            'Limpieza', 'Mantenimiento preventivo', 'Reparación urgente',
            'AJUSTE DE PRESIÓN', 'REEMPLAZO DE SELLO', 'Desgaste natural'
        ], n),
        'Comentarios': np.random.choice([
            '', 'Sin novedad', 'Requiere seguimiento', 'Urgente',
            'Programado para próxima semana', 'Completado exitosamente', 'HOLA'
        ], n)
    }
...
# ============================================================================
# 2. GEOMETRÍA DE TULIPAS POR FORMATO
# ============================================================================
...
# ============================================================================
# 3. CREAR HEATMAPS POR FORMATO Y EQUIPO
# ============================================================================
...
def crear_heatmaps(df_filtrado):
    ...
    configs = []
    for fmt in formatos_list:
        for eq in equipos_list:
            configs.append((fmt, eq))
    ...
    for fmt, eq in configs:
        ...
        matrices_all.append((matriz, fmt, eq))

    positions = [(1, 1), (1, 2), (2, 1), (2, 2), (3, 1), (3, 2), (4, 1), (4, 2)]
    zmax = max(global_max, 1)

    for idx, ((matriz, fmt, eq), (row, col)) in enumerate(zip(matrices_all, positions)):
        geo = GEOMETRIA[fmt]
        n_cabezales = geo['n_cabezales']
        n_tulipas = geo['total_tulipas']
    ...
    fig.update_layout(
        title=dict(
            text='<b>Gráfico 7. Mapa de calor por cabezal y tulipa</b>',
            font=dict(size=14),
            x=0.5,
            xanchor='center'
        ),
...
def grafico_tendencia(df):
    ...
    fig.update_layout(
        title=dict(text='<b>Gráfico 1. Tendencia temporal de mantenimientos</b>', font=dict(size=13)),
...
def grafico_formatos(df):
    ...
    fig = px.bar(
        ...
        barmode='group',
        title='<b>Gráfico 2. Registros por equipo y formato</b>',
        color_discrete_map={'237 CC': '#3498db', '350 CC': '#e74c3c', '1.000 CC': '#2ecc71', '1.250 CC': '#f39c12'},
        height=380,
...
def grafico_turno(df):
    ...
    fig.update_layout(
        title=dict(text='<b>Gráfico 3. Distribución por turno</b>', font=dict(size=13)),
        height=380,
        paper_bgcolor='rgba(0,0,0,0)',
...
def grafico_mantencion(df):
    ...
    fig.update_layout(
        title=dict(text='<b>Gráfico 4. Tipos de mantención más frecuentes</b>', font=dict(size=13)),
...
def grafico_operador(df):
    ...
    fig.update_layout(
        title=dict(text='<b>Gráfico 5. Registros por operador</b>', font=dict(size=13)),
        xaxis_title='Registros',
...
def grafico_top_tulipas(df, top_n=15):
    ...
    fig.update_layout(
        title=dict(text=f'<b>Gráfico 6. Top {top_n} tulipas más afectadas</b>', font=dict(size=13)),
        xaxis_title='Registros',
        height=480,
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(0,0,0,0)',
...
<div style='text-align:center; margin-bottom:1.2rem;'>
  ...
  <p style='font-size:0.9rem; opacity:0.7; margin:0;'>
    Elaborado por: <b>Enrique Brun</b>
  </p>
</div>
""", unsafe_allow_html=True)
...
if len(df_f) == 0:
    ...
    st.info(
        "💡 El gráfico siguiente corresponde al nivel más específico del análisis: "
        "frecuencia de eventos por combinación Cabezal × Tulipa, separada por equipo y formato."
    )
...
st.markdown(
    ...
    "<b>Dashboard Mantenimiento de Tulipas Línea 11</b> · Embotelladora CCU<br>"
